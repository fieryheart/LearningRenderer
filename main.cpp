...
void Lab() {
    // Marry Model
    QGL::Model *model = new QGL::StrangeModel("../obj/Marry.obj");
    QGL::StrangeModel *marry = NULL;
    if (model->type() == QGL::MDT_Strange) {
        marry = dynamic_cast<QGL::StrangeModel*>(model);
        marry->preprocess();
        marry->loadMap("../obj/MC003_Kozakura_Mari.png", QGL::MT_Diffuse);
        std::cout << "Model Type: " << marry->type() << std::endl;
    }
}
